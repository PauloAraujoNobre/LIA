...
class RedeNeural():
    ...
    def __init__(self, amostras, target):
        ...
        self.weights2 = np.matrix([[0.1, 0.3, 0.2], [0.3, 2, 2]])
    ...
    def backPropagation(self):
        ...
        for i in range(row):
            for j in range(col):
                d1 = output[0, (i-1)] - self.targetDegrau[i-1]
                d2 = output[0, (i-1)] * (1 - output[0, (i-1)])
                d3 = self.y[j-1]
                dt = d1 * d2 * d3

                n = dt * self.learning_rate
                self.weights2[(i-1), (j-1)] += n
        ...
        for i in range(row):
            for j in range(col):
                for k in range(col_out):
                    d11 = (output[0, (k-1)] - self.targetDegrau[j-1]) * (output[0, (k-1)] * (1 - output[0, (k-1)])) * ()
